...
def schrodinger_propagate_with_drive_rwa(
    ...
    H_drive(t) = i * [E_+(t) a + E_-(t) a^?],
    with E_+(t) = 0.5 E0(t) e^{+i omega t}, E_-(t) = 0.5 E0(t) e^{-i omega t}.
    """
    times_aut = fs_to_aut(times_fs)
    ...
    for i in range(len(times_fs) - 1):
        ...
        # H_drive = i [E_+(t) a + E_-(t) a^?]
        H_drive = 0.5j * env * (- phase * drive_op_minus + phase.conj() * drive_op_plus)
...
if __name__ == "__main__":
    ...
    # Cavity-drive operator coupling |G> <-> |C> for driving
    ...
    a_op_plus_elec = csr_matrix(np.outer(e_c, e_g))
    a_op_minus_elec = csr_matrix(np.outer(e_g, e_c))
    drive_op_plus = kron(a_op_plus_elec, I_v, format='csr')
    drive_op_minus = kron(a_op_minus_elec, I_v, format='csr')
    ...
    pulse_freq_eV = 2.0
    # pulse_amplitude = eV_to_Hartree(8e-3)  # weak field (Hartree)
    # pulse_amplitude = eV_to_Hartree(0.525)  # 0.9 pi pulse
    pulse_amplitude = eV_to_Hartree(0.4)  # pi pulse
