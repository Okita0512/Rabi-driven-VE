...
omega_v_eV  = 0.19866      # vibrational frequency [eV] (used if vib_trunc>=2)
...
E_photon_eV      = omega_c_eV * photon_num
E_exciton_eV     = N * omega_0_eV * excited_pop
E_vibrational_eV = Evib_total_eV

# interaction energies [eV] for the full MF Hamiltonian
sigma_minus_t      = np.array([expect(SigmaMinus, psi) for psi in psi_t], dtype=complex)
Xe_proj_e_t        = np.array([expect(Proj_e @ (B + Bd), psi) for psi in psi_t], dtype=complex)
E_light_matter_eV  = 2.0 * HBAR_eVfs * g * N * np.real(np.conj(alpha_t) * sigma_minus_t)
E_holstein_eV      = HBAR_eVfs * c_v * N * np.real(Xe_proj_e_t)
E_total_eV         = E_photon_eV + E_exciton_eV + E_vibrational_eV + E_light_matter_eV + E_holstein_eV
# E_total_eV         = E_photon_eV + E_exciton_eV + E_vibrational_eV #+ E_light_matter_eV + E_holstein_eV
# E_total_eV         = E_light_matter_eV + E_holstein_eV
...
# ---------------- save to disk ----------------
np.savetxt("Excited_population.dat", np.column_stack([t_save, excited_pop]))
np.savetxt("Photon_number.dat",      np.column_stack([t_save, photon_num]))
np.savetxt("Xg.dat",                 np.column_stack([t_save, Xg_eV]))
np.savetxt("Vg.dat",                 np.column_stack([t_save, Vg_eV]))

np.savetxt("Vib_pops_g.dat", np.column_stack([t_save, vib_pops_g]))
np.savetxt("Vib_pops_e.dat", np.column_stack([t_save, vib_pops_e]))

# (Optional) keep per-molecule file if you still want it
np.savetxt(
    "Evib_per_molecule.dat",
    np.column_stack([t_save, Evib_per_mol_eV, Evib_g_per_mol_eV, Evib_e_per_mol_eV])
)

# energy traces [eV]
np.savetxt("Photon_energy.dat",      np.column_stack([t_save, E_photon_eV]))
np.savetxt("Exciton_energy.dat",     np.column_stack([t_save, E_exciton_eV]))
np.savetxt("Vibrational_energy.dat", np.column_stack([t_save, E_vibrational_eV]))
np.savetxt("Total_energy.dat",       np.column_stack([t_save, E_total_eV]))

# combined file for quick post-processing:
# columns: t(fs), total, exciton, vibrational, photon, light-matter, holstein
np.savetxt(
    "Energy_components.dat",
    np.column_stack([
        t_save,
        E_total_eV,
        E_exciton_eV,
