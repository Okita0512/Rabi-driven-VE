...
g_eV        = 0.155 / np.sqrt(N)      # light–matter coupling [eV]
...
# ---------------- save to disk ----------------
save_stride = 10  # save every N time points
save_idx = slice(None, None, save_stride)
t_save = sol.t[save_idx]
np.savetxt("Excited_population.dat", np.column_stack([t_save, excited_pop[save_idx]]))
np.savetxt("Photon_number.dat",      np.column_stack([t_save, photon_num[save_idx]]))
np.savetxt("Xg.dat",                 np.column_stack([t_save, Xg_eV[save_idx]]))
np.savetxt("Vg.dat",                 np.column_stack([t_save, Vg_eV[save_idx]]))

np.savetxt("Vib_pops_g.dat", np.column_stack([t_save, vib_pops_g[save_idx]]))
np.savetxt("Vib_pops_e.dat", np.column_stack([t_save, vib_pops_e[save_idx]]))  # NEW

# NEW: total vibrational energy (total, g, e) vs time
# columns: t, E_vib_total, E_vib_g, E_vib_e
# np.savetxt(
#     "Evib_total.dat",
#     np.column_stack([t_save, Evib_total_eV[save_idx], Evib_g_total_eV[save_idx], Evib_e_total_eV[save_idx]])
# )

# (Optional) keep per-molecule file if you still want it
np.savetxt(
    "Evib_per_molecule.dat",
    np.column_stack([t_save, Evib_per_mol_eV[save_idx], Evib_g_per_mol_eV[save_idx], Evib_e_per_mol_eV[save_idx]])
)
