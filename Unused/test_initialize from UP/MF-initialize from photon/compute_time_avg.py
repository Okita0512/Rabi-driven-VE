...
def main():
    ...
    for inten, folder in entries:
        ...
        if os.path.isfile(evib_path):
            evib_cols = ["time_fs", "Evib_per_molecule", "Evib_per_molecule_g", "Evib_per_molecule_e"]
            evib = read_table(evib_path, evib_cols)
            evib = filter_time(evib)
            E_vib.append({
                "Rabi_meV": inten,
                "Evib_per_molecule_avg": evib["Evib_per_molecule"].mean(),
                "Evib_per_molecule_g_avg": evib["Evib_per_molecule_g"].mean(),
                "Evib_per_molecule_e_avg": evib["Evib_per_molecule_e"].mean(),
            })
        else:
