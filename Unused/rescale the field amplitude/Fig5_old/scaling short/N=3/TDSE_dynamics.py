...
def schrodinger_propagate_with_drive_rwa(
    ...
    H_drive(t) = i [a^dag E_-(t) - a E_+(t)], with E_+(t)=0.5 E0(t) e^{-i omega t}, E_-(t)=0.5 E0(t) e^{+i omega t}.
    """
    times_aut = fs_to_aut(times_fs)
...
if __name__ == "__main__":
    ...
    adag_op_elec = csr_matrix(np.outer(e_c, e_g))   # a^dag = |C><G|
    ...
    #pulse_freq_eV = 2.1
    # ==================
    pulse_amplitude = eV_to_Hartree(1e-3 * np.sqrt(N))  # weak field (Hartree)
