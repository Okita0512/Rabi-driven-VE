...
fig = plt.figure(figsize=(25, 5), dpi = 128)
...
data = np.loadtxt(os.path.join("Scaling long (pump UP)", "N=1", "Vib_level_pop.dat"))
plt.plot(data[:,0], 0 * 5e-3 + data[:,5], '-', label = r"$N$ = 1")

data = np.loadtxt(os.path.join("Scaling long (pump UP)", "N=2", "Vib_level_pop.dat"))
plt.plot(data[:,0], 1 * 5e-3 + data[:,5], '-', label = r"$N$ = 2")

data = np.loadtxt(os.path.join("Scaling long (pump UP)", "N=4", "Vib_level_pop.dat"))
plt.plot(data[:,0], 2 * 5e-3 + data[:,5], '-', label = r"$N$ = 4")

data = np.loadtxt(os.path.join("Scaling long (pump UP)", "N=8", "Vib_level_pop.dat"))
plt.plot(data[:,0], 3 * 5e-3 + data[:,5], '-', label = r"$N$ = 8")
...
y1, y2 = 0, 2.5e-2     # y-axis range: (y1, y2)
...
y_major_locator = MultipleLocator(5e-3)
y_minor_locator = MultipleLocator(5e-3)
...
# ==============================================================================================
plt.subplot(1, 4, 2)
path_base = "Scaling long (pump UP)"
Ns = [1,2,3,4,5,6,7,8]
...
for n in Ns:
    ...
    y = arr[:,5]
    # Skip initial flat region before the field turns on.
    # P_e_n1 is a population (slow Rabi oscillations), so peaks are found directly.
    ymax = np.nanmax(y)
    if ymax <= 0:
        periods.append(np.nan)
        continue
    start_idx = int(np.argmax(y >= 0.05 * ymax))
    period = estimate_period_from_time(t[start_idx:], y[start_idx:])
    periods.append(period)
periods = np.array(periods)
print("Panel (b) extracted periods:")
for n, T in zip(Ns, periods):
    print(f"  N={n}: T = {T:.1f} fs")
...
log10_periods = np.log10(periods[mask])
log10_Ns = np.log10(Ns_arr[mask])
y_pred = slope * log10_Ns + intercept
ss_res = np.sum((log10_periods - y_pred) ** 2)
ss_tot = np.sum((log10_periods - np.mean(log10_periods)) ** 2)
r2 = 1 - ss_res / ss_tot if ss_tot > 0 else 1.0
print(f"FigS11 panel b fit results:")
print(f"  T fit: slope={slope:.3f}, intercept={intercept:.3f}, R^2={r2:.4f}")
N_fit = np.linspace(1, 8, 200)
T_fit = 10**intercept * N_fit**slope
plt.plot(N_fit, T_fit, '--', color='gray', linewidth=1.5)

# Reference sqrt(N) line anchored at N=1 for comparison
if not np.isnan(periods[0]):
    N_sqrt = np.linspace(1, 8, 200)
    plt.plot(N_sqrt, periods[0] * np.sqrt(N_sqrt), ':', color='black', linewidth=1.5)
...
if fit_results:
    print("FigS11 panel c fit results:")
...
if fit_results:
    print("FigS11 panel d fit results:")
    for entry in fit_results:
        print("  " + entry)



plt.savefig("FigS11.pdf", bbox_inches='tight')
