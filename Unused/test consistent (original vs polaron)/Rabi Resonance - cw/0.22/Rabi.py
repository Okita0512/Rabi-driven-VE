...
g_eV        = 0.11 / np.sqrt(N)      # light–matter coupling [eV]
c_v_eV      = 0.02       # Holstein coupling [eV], set 0 for clean Rabi
# c_v_eV      = 0.0       # Holstein coupling [eV], set 0 for clean Rabi

# Representation control: "original" (Holstein, no displacement) or "polaron" (displaced, no Holstein)
representation = "original"
# representation = "polaron"
...
kappa_rate     = 0.02     # cavity loss [1/fs]
...
# CW laser (matter dipole drive)
# pulse_omega_eV  = 2.1        # drive frequency [eV]
...
lam     = (c_v ** 2) / omega_v if omega_v != 0.0 else 0.0

if representation == "original":
    Delta_x_eff = Delta_x + lam
    use_displacement = False
    include_holstein = True
elif representation == "polaron":
    Delta_x_eff = Delta_x
    use_displacement = True
    include_holstein = False
else:
    raise ValueError("representation must be 'original' or 'polaron'")
...
if use_displacement and d_v > 1 and omega_v != 0.0:
    # Franck-Condon dressing via displacement operator D(-d), d = c_v / omega_v
    d_fc = c_v / omega_v
    D_fc_vib = expm(d_fc * (adag - a))
else:
    D_fc_vib = Iv

SigmaMinus = kron(sig_minus, D_fc_vib.conj().T)
SigmaPlus  = kron(sig_plus, D_fc_vib)
...
def pulse_drive(t):
    if Omega_0 == 0.0:
        return 0.0
    return Omega_0 * np.exp(-1j * Delta_p * t)

def H_mol(alpha, t):
    H = Delta_x_eff * Proj_e + omega_v * Nhat
    if include_holstein and c_v != 0.0:
        H = H + c_v * Proj_e @ (B + Bd)
