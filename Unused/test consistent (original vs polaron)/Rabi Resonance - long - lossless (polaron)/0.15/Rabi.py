...
omega_v_eV  = 0.150       # vibrational frequency [eV] (used if vib_trunc>=2)
...
pulse_t0_fs     = 1.4 * 250.0      # pulse center [fs]
pulse_sigma_fs  = 1.4 * 50.0       # pulse width [fs]
...
def rhs(t, y):
    ...
    # 1. Molecule master equation (Already correct in your script)
    H  = H_mol(alpha, t)
    drho = -1j*(H @ rho - rho @ H)
    for L in collapses:
        drho += D(L, rho)
    
    pol_source = expect(SigmaMinus, rho)

    dalpha = -(kappa_rate/2 + 1j*Delta_c)*alpha - 1j * g * N * pol_source + eta
