...
pulse_amp_eV    = 4.0e-3 / np.sqrt(N)    # field amplitude [eV] (Rabi energy scale)
...
np.savetxt("Excited_population.dat", np.column_stack([t_save, excited_pop[::save_stride]]))
np.savetxt("Photon_number.dat",      np.column_stack([t_save, photon_num[::save_stride]]))
np.savetxt("Xg.dat",                 np.column_stack([t_save, Xg_eV[::save_stride]]))
np.savetxt("Vg.dat",                 np.column_stack([t_save, Vg_eV[::save_stride]]))

np.savetxt("Vib_pops_g.dat", np.column_stack([t_save, vib_pops_g[::save_stride, :]]))
np.savetxt("Vib_pops_e.dat", np.column_stack([t_save, vib_pops_e[::save_stride, :]]))  # NEW
...
np.savetxt(
    ...
    np.column_stack([t_save, Evib_per_mol_eV[::save_stride], Evib_g_per_mol_eV[::save_stride], Evib_e_per_mol_eV[::save_stride]])
)
