...
if __name__ == "__main__":
    ...
    # Dipole operator coupling |G> <-> |B> (symmetric exciton) for driving
    I_v = identity(info["dim_vib"], dtype=np.complex128, format='csr')
    e_g = psi0_e
    e_b = np.zeros((dim_e,), dtype=np.complex128); e_b[2:] = 1.0 / np.sqrt(N)
    mu_op_elec = csr_matrix(np.outer(e_b, e_g) + np.outer(e_g, e_b))
    drive_op = kron(mu_op_elec, I_v, format='csr')
    ...
    # Propagate
    Psi_t = schrodinger_propagate_with_drive(H, drive_op, pulse_envelope, psi0, times_fs)
