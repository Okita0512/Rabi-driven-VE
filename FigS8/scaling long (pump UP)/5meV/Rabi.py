...
def rhs(t, y):
    ...
    dalpha = -(kappa_rate/2 + 1j*Delta_c)*alpha - 1j * g * N * sigma_minus + eta + Omega_t
...
# downsample saves (every 10 points)
idx_save = np.arange(0, len(sol.t), 10)
t_save = sol.t[idx_save]
# ---------------- save to disk ----------------
np.savetxt("Excited_population.dat", np.column_stack([t_save, excited_pop[idx_save]]))
np.savetxt("Photon_number.dat",      np.column_stack([t_save, photon_num[idx_save]]))
np.savetxt("Xg.dat",                 np.column_stack([t_save, Xg_eV[idx_save]]))
np.savetxt("Vg.dat",                 np.column_stack([t_save, Vg_eV[idx_save]]))

np.savetxt("Vib_pops_g.dat", np.column_stack([t_save, vib_pops_g[idx_save]]))
np.savetxt("Vib_pops_e.dat", np.column_stack([t_save, vib_pops_e[idx_save]]))  # NEW

# NEW: total vibrational energy (total, g, e) vs time
# columns: t, E_vib_total, E_vib_g, E_vib_e
# np.savetxt(
#     "Evib_total.dat",
#     np.column_stack([t_save, Evib_total_eV[idx_save], Evib_g_total_eV[idx_save], Evib_e_total_eV[idx_save]])
# )

# (Optional) keep per-molecule file if you still want it
np.savetxt(
    "Evib_per_molecule.dat",
    np.column_stack([t_save, Evib_per_mol_eV[idx_save], Evib_g_per_mol_eV[idx_save], Evib_e_per_mol_eV[idx_save]])
)
