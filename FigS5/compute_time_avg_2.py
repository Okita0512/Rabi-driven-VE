...
ENERGY_OUT = os.path.join(BASE_DIR, "energy_summary_2.csv")
POP_OUT = os.path.join(BASE_DIR, "pop_summary_2.csv")
TIME_START_FS = 0.0
TIME_End_FS = 50.0
...
def filter_time(df, time_col="time_fs", tmin=TIME_START_FS, tmax=TIME_End_FS):
    return df[(df[time_col] > tmin) & (df[time_col] < tmax)]
...
def main():
    ...
    for inten, folder in entries:
        ...
        ekin_path = os.path.join(folder, "Ekin.dat")
        ekin_tot = np.nan
        ekin_g = np.nan
        ekin_e = np.nan
        if os.path.isfile(ekin_path):
            ekin_cols = ["time_fs", "Ekin_total", "Ekin_ground", "Ekin_excited"]
            ekin = read_table(ekin_path, ekin_cols)
            ekin = filter_time(ekin)
            ekin_tot = ekin["Ekin_total"].mean()
            ekin_g = ekin["Ekin_ground"].mean()
            ekin_e = ekin["Ekin_excited"].mean()
        else:
            print(f"Skip {folder}: missing Ekin.dat (kinetic energy averages set to NaN)")
        ...
        energy_rows.append({
            ...
            "Ekin_ground_avg": ekin_g,
            "Ekin_excited_avg": ekin_e,
            "Ekin_total_avg": ekin_tot,
        })
