...
# ---------------- user parameters ----------------
N           = 105.0625         # number of identical, uncorrelated molecules
...
g_eV        = 0.01      # light–matter coupling [eV]
...
eta            = 0.0 + 0.0j  # external cavity drive [1/fs]; keep 0 unless desired
seed_alpha     = 0.0         # tiny seed to trigger Rabi in mean-field
# init_vib_n: int -> Fock |n>, or "shifted"/"displaced"/"coherent" -> displaced vacuum
init_vib_n     = 0
init_vib_shift = 0.0         # coherent displacement amplitude (small shift around equilibrium)
...
# ---------------- initial condition: molecule in |g,v>, cavity tiny seed ----------------
ket_e = np.array([0,1], dtype=complex)
ket_g = np.array([1,0], dtype=complex)

def _coherent_state(alpha, dim):
    coeffs = np.array(
        [alpha**n / math.sqrt(math.factorial(n)) for n in range(dim)],
        dtype=complex,
    )
    ket = np.exp(-0.5 * (abs(alpha)**2)) * coeffs
    norm = np.linalg.norm(ket)
    return ket if norm == 0.0 else ket / norm

if isinstance(init_vib_n, (int, np.integer)):
    if init_vib_n < 0 or init_vib_n >= d_v:
        raise ValueError(f"init_vib_n={init_vib_n} out of range for vib_trunc={d_v}")
    ket_v = np.zeros((d_v,), dtype=complex)
    ket_v[init_vib_n] = 1.0
elif isinstance(init_vib_n, str) and init_vib_n.lower() in ("shifted", "displaced", "coherent"):
    ket_v = _coherent_state(init_vib_shift, d_v)
else:
    raise ValueError("init_vib_n must be int or one of: 'shifted', 'displaced', 'coherent'")
psie = kron(ket_e, ket_v)             # |e,v>
psig = kron(ket_g, ket_v)             # |g,v>
...
P2_avg   = np.real([expect(P2, rho) for rho in rho_t])
P2_g_avg = np.real([expect(Proj_g @ P2, rho) for rho in rho_t])
P2_e_avg = np.real([expect(Proj_e @ P2, rho) for rho in rho_t])

# kinetic energies (per molecule), in eV
Ekin_per_mol_eV    = (omega_v * P2_avg / 4.0)   * HBAR_eVfs
Ekin_g_per_mol_eV  = (omega_v * P2_g_avg / 4.0) * HBAR_eVfs
Ekin_e_per_mol_eV  = (omega_v * P2_e_avg / 4.0) * HBAR_eVfs

# total (ensemble) kinetic energies [eV]
Ekin_total_eV      = N * Ekin_per_mol_eV
Ekin_g_total_eV    = N * Ekin_g_per_mol_eV
Ekin_e_total_eV    = N * Ekin_e_per_mol_eV
...
# total kinetic energy (total, g, e) vs time
# columns: t, E_kin_total, E_kin_g, E_kin_e
np.savetxt(
    "Ekin.dat",
    np.column_stack([sol.t, Ekin_total_eV, Ekin_g_total_eV, Ekin_e_total_eV])
)
