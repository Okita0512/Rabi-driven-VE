...
def schrodinger_propagate_with_drive_rwa(
    ...
    psi0,
    times_fs: np.ndarray,
    phase_center_fs: float = 0.0,
    dtype=np.complex128,
):
    """
    Midpoint Trotter with RWA drive:
    H_drive(t) = 0.5 * E0(t) * [e^{-i omega (t-t0)} V_plus + e^{+i omega (t-t0)} V_minus].
    """
    times_aut = fs_to_aut(times_fs)
    ...
    for i in range(len(times_fs) - 1):
        ...
        t_mid_aut = fs_to_aut(t_mid_fs - phase_center_fs)
...
if __name__ == "__main__":
    ...
    omega_v = eV_to_Hartree(0.09)
    g_c     = eV_to_Hartree(0.20 / (2 * np.sqrt(N)))  # your choice here
    ...
    # Cavity-field coupling: H_pulse(t) = i*hbar( a^dagger E_- - a E_+ )
    # a lowers |C> -> |G|; a^dagger raises |G> -> |C>
    I_v = identity(info["dim_vib"], dtype=np.complex128, format='csr')
    e_g = psi0_e
    e_c = np.zeros((dim_e,), dtype=np.complex128); e_c[1] = 1.0
    a_op_elec = csr_matrix(np.outer(e_g, e_c))      # |G><C|
    adag_op_elec = csr_matrix(np.outer(e_c, e_g))   # |C><G|
    drive_op_plus = kron(-1j * a_op_elec, I_v, format='csr')
    drive_op_minus = kron(1j * adag_op_elec, I_v, format='csr')

    # Bright exciton |B> for dark-state projector
    ...
    Psi_t = schrodinger_propagate_with_drive_rwa(
        ...
        psi0,
        times_fs,
        phase_center_fs=pulse_center_fs,
    )
