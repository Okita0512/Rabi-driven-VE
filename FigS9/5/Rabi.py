...
# ---------------- user parameters ----------------
N           = 100000       # number of identical, uncorrelated molecules
...
pulse_amp_eV    = 1.0e-03 # * np.sqrt(N)    # field amplitude [eV] (Rabi energy scale)
...
def rhs(t, y):
    ...
    dalpha = -(kappa_rate/2 + 1j*Delta_c)*alpha - 1j * g * N * sigma_minus + eta + eta_t
...
# ---------------- save to disk ----------------
np.savetxt("Excited_population.dat", np.column_stack([sol.t[::save_stride], excited_pop[::save_stride]]))
np.savetxt("Photon_number.dat",      np.column_stack([sol.t[::save_stride], photon_num[::save_stride]]))
np.savetxt("Xg.dat",                 np.column_stack([sol.t[::save_stride], Xg_eV[::save_stride]]))
np.savetxt("Vg.dat",                 np.column_stack([sol.t[::save_stride], Vg_eV[::save_stride]]))

np.savetxt("Vib_pops_g.dat", np.column_stack([sol.t[::save_stride], vib_pops_g[::save_stride]]))
np.savetxt("Vib_pops_e.dat", np.column_stack([sol.t[::save_stride], vib_pops_e[::save_stride]]))  # NEW

# NEW: total vibrational energy (total, g, e) vs time
# columns: t, E_vib_total, E_vib_g, E_vib_e
# np.savetxt(
#     "Evib_total.dat",
#     np.column_stack([sol.t[::save_stride], Evib_total_eV[::save_stride], Evib_g_total_eV[::save_stride], Evib_e_total_eV[::save_stride]])
# )

# (Optional) keep per-molecule file if you still want it
np.savetxt(
    "Evib_per_molecule.dat",
    np.column_stack([sol.t[::save_stride], Evib_per_mol_eV[::save_stride], Evib_g_per_mol_eV[::save_stride], Evib_e_per_mol_eV[::save_stride]])
)
